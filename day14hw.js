window.addEventListener('load', function () {
    ...
    request.onload = function() {
       // console.log(request.responseText);
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            document.getElementById("textBox"); //the below code doesn't work in this spot --> needs to be at the appendChild line. ".innerHTML = data[i].user + ': ' + data[i].message;"
            textBox.appendChild(chatOutput).innerHTML = '[' + data[i].user + ']' + ': ' + data[i].message;
        } //end for
    ...
    request.onload = function() {
       ...
       // console.log(request.responseText);
        ...
        var data = JSON.parse(request.responseText);
        for (var i = 0; i < data.length; i++) {
            var chatParent = document.createElement('li');
            chatParent.textContent = data[i];
            document.getElementById("textBox");
            textBox.appendChild(chatParent).innerHTML = '[' + data[i].user + ']' + ': ' + data[i].message;
        } // end for
        console.log(data);// this has a value now!
    };
    // actually make the request.
    request.send();
  });
